...
let
  ...
  sajeda-pdf-static = stdenv.mkDerivation {
    pname = "sajeda-pdf-static";
    version = "0.1.0";
...
stdenv.mkDerivation {
  ...
  installPhase = let
    nginxConf = builtins.toFile "sajeda-pdf-nginx.conf" ''
      daemon off;
      worker_processes 1;
  ...
  in ''
    ...
    mkdir -p $out/share/sajeda-pdf
    cp -r ${sajeda-pdf-static}/* $out/share/sajeda-pdf/

    # Decompress LibreOffice WASM .gz files
    if [ -d $out/share/sajeda-pdf/libreoffice-wasm ]; then
      cd $out/share/sajeda-pdf/libreoffice-wasm
      for f in *.gz; do
        if [ -f "$f" ]; then
    ...
    mkdir -p $out/etc/sajeda-pdf
    sed -e "s|PDFCRAFT_ROOT|$out/share/sajeda-pdf|g" \
        -e "s|NGINX_MIME_TYPES|${nginx}/conf/mime.types|g" \
        ${nginxConf} > $out/etc/sajeda-pdf/nginx.conf

    # Install run script
    mkdir -p $out/bin
    cat > $out/bin/sajeda-pdf <<'WRAPPER'
#!/bin/sh
PDFCRAFT_PORT=''${PDFCRAFT_PORT:-3000}
PDFCRAFT_CONF="@out@/etc/sajeda-pdf/nginx.conf"
RUNTIME_CONF=$(mktemp /tmp/sajeda-pdf-nginx.XXXXXX.conf)

sed "s|listen 3000|listen $PDFCRAFT_PORT|g" "$PDFCRAFT_CONF" > "$RUNTIME_CONF"

trap "rm -f $RUNTIME_CONF" EXIT

echo "Sajeda PDF running at http://localhost:$PDFCRAFT_PORT"
exec @nginx@/bin/nginx -c "$RUNTIME_CONF"
WRAPPER

    substituteInPlace $out/bin/sajeda-pdf \
      --replace-fail "@out@" "$out" \
      --replace-fail "@nginx@" "${nginx}"
    chmod +x $out/bin/sajeda-pdf
  ...
  meta = with lib; {
    ...
    homepage = "https://github.com/sajedapdf/sajeda-pdf";
    license = licenses.agpl3Only;
    platforms = [ "x86_64-linux" "aarch64-linux" ];
    mainProgram = "sajeda-pdf";
  };
}
